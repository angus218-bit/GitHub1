...
#
# This script is the local docs/config gate. App tests live in
# apps/demo and apps/family-hearth and are run by .github/workflows/ci.yml.
# Contributors can run this before pushing:
#
...
if command -v markdownlint >/dev/null 2>&1; then
  ...
  if [ "${#md_files[@]}" -eq 0 ]; then
    skip "no Markdown files tracked"
  elif markdownlint --config .markdownlint.jsonc "${md_files[@]}"; then
